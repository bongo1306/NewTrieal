...
class DesignEngineeringHours(PlotPanel):
	def __init__( self, parent, start_date, end_date, **kwargs ):
		...
		for employee_index, employee in enumerate(design_engineers):
			...
			self.employee_data.append( (
					...
					cursor.execute('SELECT when_logged, hours FROM time_logs WHERE employee=\'{}\' AND when_logged>\'{}\' AND when_logged<\'{}\' ORDER BY when_logged ASC'.format(employee.replace("'", "''"), self.start_date, '{} 23:59:59'.format(self.end_date))).fetchall())
				)
	...
	def draw(self):
		...
		for index, data in enumerate(self.employee_data):
			...
			self.subplot.annotate(employee.split(',')[0], xy=(x[annotation_x_index], y[annotation_y_index]), xytext=(-15,15), 
...
class EcrReasons(PlotPanel):
	def __init__( self, parent, start_date, end_date, **kwargs ):
		...
		for reason_index, reason in enumerate(ecr_reasons):
			self.reason_data.append( (
					reason, 
					cursor.execute('SELECT when_closed FROM ecrs WHERE reason=\'{}\' AND when_closed>\'{}\' AND when_closed<\'{}\' ORDER BY when_closed ASC'.format(reason, self.start_date, '{} 23:59:59'.format(self.end_date))).fetchall())
					)
	...
	def draw(self):
		...
		for index, data in enumerate(self.reason_data):
			...
			self.subplot.annotate(reason.split(',')[0], xy=(x[annotation_x_index], y[annotation_y_index]), xytext=(-15,15), 
						textcoords='offset points', ha='center', va='bottom',
						bbox=dict(boxstyle='round,pad=0.2', fc='yellow', alpha=0.5),
						arrowprops=dict(arrowstyle='->', connectionstyle='arc3', color='black'))
...
class EcrsClosedOnTime(PlotPanel):
	def __init__( self, parent, start_date, end_date, **kwargs ):
		...
		self.ecr_data = cursor.execute("SELECT when_needed, when_closed FROM ecrs WHERE when_closed>\'{}\' AND when_closed<\'{}\' ORDER BY when_closed ASC".format(self.start_date, '{} 23:59:59'.format(self.end_date))).fetchall()
	...
	def draw(self):
		...
		#show goal line (93%)
		self.subplot.plot_date([self.lowest_start_date, self.end_date + relativedelta(hours=17)], [.93*100, .93*100], '-')
		...
		self.subplot.set_title(r"ECRs Closed On Time   ({:.2f}% of {} ECRs)".format(float(on_time_sum / total_sum)*100, int(total_sum)))
...
class EcrsByProductFamily(PlotPanel):
	def __init__( self, parent, start_date, end_date, **kwargs ):
		...
		try:
			families = list(zip(*cursor.execute('SELECT {}.family FROM ecrs INNER JOIN {} ON ecrs.item = {}.item WHERE ecrs.when_closed>\'{}\' AND ecrs.when_closed<\'{}\' ORDER BY {}.family ASC'.format(Ecrs.table_used, Ecrs.table_used, Ecrs.table_used, self.start_date, '{} 23:59:59'.format(self.end_date), Ecrs.table_used)).fetchall())[0])
		except:
			families = []
...
class EcrsByCustomer(PlotPanel):
	def __init__( self, parent, start_date, end_date, **kwargs ):
		...
		try:
			customers = list(zip(*cursor.execute('SELECT {}.customer FROM ecrs INNER JOIN {} ON ecrs.item = {}.item WHERE ecrs.when_closed>\'{}\' AND ecrs.when_closed<\'{}\' ORDER BY {}.customer ASC'.format(Ecrs.table_used, Ecrs.table_used, Ecrs.table_used, self.start_date, '{} 23:59:59'.format(self.end_date), Ecrs.table_used)).fetchall())[0])
		except:
			customers = []
		...
		for customer_index, customer in enumerate(customers):
			self.customer_data.append( (
					...
					#cursor.execute('SELECT when_closed FROM ecrs WHERE document=\'{}\' AND when_closed>\'{}\' AND when_closed<\'{}\' ORDER BY when_closed ASC'.format(document, self.start_date, '{} 23:59:59'.format(self.end_date))).fetchall())
					#cursor.execute('SELECT when_closed FROM ecrs WHERE document=\'{}\' AND when_closed>\'{}\' AND when_closed<\'{}\' AND (resolution LIKE \'%picklist%\' OR resolution LIKE \'%pick list%\' OR request LIKE \'%picklist%\' OR request LIKE \'%pick list%\') ORDER BY when_closed ASC'.format(document, self.start_date, '{} 23:59:59'.format(self.end_date))).fetchall())
					cursor.execute('SELECT ecrs.when_closed FROM ecrs INNER JOIN {} ON ecrs.item = {}.item WHERE {}.customer=\'{}\' AND ecrs.when_closed>\'{}\' AND ecrs.when_closed<\'{}\' ORDER BY ecrs.when_closed ASC'.format(Ecrs.table_used, Ecrs.table_used, Ecrs.table_used, customer, self.start_date, '{} 23:59:59'.format(self.end_date))).fetchall())
				)
	...
	def draw(self):
		...
		#self.subplot.legend(bbox_to_anchor=(0, 1), loc=2, borderaxespad=.8, prop={'size':10})
		#self.subplot.legend(bbox_to_anchor=(0, 1), loc=2, fancybox=True, borderaxespad=.8, prop={'size':self.figure.get_figheight() * 1.4})
		self.subplot.legend(bbox_to_anchor=(0, 1), loc=2, fancybox=True, borderaxespad=.8, prop={'size':self.figure.get_figheight() * 1})
...
class HoursLoggedByProductFamily(PlotPanel):
	def __init__( self, parent, start_date, end_date, **kwargs ):
		...
		families = list(zip(*cursor.execute('SELECT {}.family FROM time_logs INNER JOIN {} ON time_logs.item = {}.item WHERE time_logs.when_logged>\'{}\' AND time_logs.when_logged<\'{}\' ORDER BY {}.family ASC'.format(Ecrs.table_used, Ecrs.table_used, Ecrs.table_used, self.start_date, '{} 23:59:59'.format(self.end_date), Ecrs.table_used)).fetchall())[0])
		#remove duplicate families from list
		families = list(set(families))
		families.sort()
		#print families
		...
		for family_index, family in enumerate(families):
			self.family_data.append( (
					...
					#cursor.execute('SELECT when_closed FROM ecrs WHERE document=\'{}\' AND when_closed>\'{}\' AND when_closed<\'{}\' ORDER BY when_closed ASC'.format(document, self.start_date, '{} 23:59:59'.format(self.end_date))).fetchall())
					#cursor.execute('SELECT when_closed FROM ecrs WHERE document=\'{}\' AND when_closed>\'{}\' AND when_closed<\'{}\' AND (resolution LIKE \'%picklist%\' OR resolution LIKE \'%pick list%\' OR request LIKE \'%picklist%\' OR request LIKE \'%pick list%\') ORDER BY when_closed ASC'.format(document, self.start_date, '{} 23:59:59'.format(self.end_date))).fetchall())
					cursor.execute('SELECT time_logs.when_logged, time_logs.hours FROM time_logs INNER JOIN {} ON time_logs.item = {}.item WHERE {}.family=\'{}\' AND time_logs.when_logged>\'{}\' AND time_logs.when_logged<\'{}\' ORDER BY time_logs.when_logged ASC'.format(Ecrs.table_used, Ecrs.table_used, Ecrs.table_used, family, self.start_date, '{} 23:59:59'.format(self.end_date))).fetchall())
				)
	...
	def draw(self):
		...
		#self.subplot.legend(bbox_to_anchor=(0, 1), loc=2, borderaxespad=.8, prop={'size':10})
		#self.subplot.legend(bbox_to_anchor=(0, 1), loc=2, fancybox=True, borderaxespad=.8, prop={'size':self.figure.get_figheight() * 1.4})
		self.subplot.legend(bbox_to_anchor=(0, 1), loc=2, fancybox=True, borderaxespad=.8, prop={'size':self.figure.get_figheight() * .9})
...
def on_click_radio(event):
	...
	date_wx_format = wx.DateTimeFromDMY(General.app.start_date.day, General.app.start_date.month-1, General.app.start_date.year)
	ctrl(General.app.main_frame, 'date:report_start').SetValue(date_wx_format)

	date_wx_format = wx.DateTimeFromDMY(General.app.end_date.day, General.app.end_date.month-1, General.app.end_date.year)
...
def refresh_plots():
	...
	try:
		...
		if General.app.report_name == "DE's Logged Hours":
			General.app.plotting_panel = DesignEngineeringHours(ctrl(General.app.main_frame, 'panel:plot'), General.app.start_date, General.app.end_date)
		if General.app.report_name == "ECR Reasons":
			General.app.plotting_panel = EcrReasons(ctrl(General.app.main_frame, 'panel:plot'), General.app.start_date, General.app.end_date)
		if General.app.report_name == "ECR Documents":
			General.app.plotting_panel = EcrDocuments(ctrl(General.app.main_frame, 'panel:plot'), General.app.start_date, General.app.end_date)
		if General.app.report_name == "ECRs Closed On Time":
			General.app.plotting_panel = EcrsClosedOnTime(ctrl(General.app.main_frame, 'panel:plot'), General.app.start_date, General.app.end_date)
		if General.app.report_name == "ECRs by Product Family":
			General.app.plotting_panel = EcrsByProductFamily(ctrl(General.app.main_frame, 'panel:plot'), General.app.start_date, General.app.end_date)
		if General.app.report_name == "ECRs by Customer":
			General.app.plotting_panel = EcrsByCustomer(ctrl(General.app.main_frame, 'panel:plot'), General.app.start_date, General.app.end_date)
		if General.app.report_name == "Hours Logged by Product Family":
			General.app.plotting_panel = HoursLoggedByProductFamily(ctrl(General.app.main_frame, 'panel:plot'), General.app.start_date, General.app.end_date)
