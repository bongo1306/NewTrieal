...
class ECRevApp(wx.App):
	...
	def init_assign_ecr_dialog(self, ecr_id):
		...
		people = list(zip(*cursor.execute('SELECT name FROM employees WHERE department = \'Design Engineering\' OR department = \'Applications Engineering\' ORDER BY name ASC').fetchall())[0])
		people.insert(0, '')
	...
	def init_main_frame(self):
		...
		toolbar.AddLabelTool(id=9990, label='Log Out', bitmap=wx.Bitmap(General.resource_path('icons\system-log-out.png')))
		...
		can_approve_first, can_approve_second = cursor.execute("SELECT can_approve_first, can_approve_second FROM employees WHERE name = '{}'".format(self.current_user)).fetchone()
	...
	def init_close_ecr_dialog(self, close_ecr_id):
		...
		can_approve_first = cursor.execute("SELECT can_approve_first FROM employees WHERE name = '{}'".format(self.current_user)).fetchone()[0]
		...
		can_approve_second = cursor.execute("SELECT can_approve_second FROM employees WHERE name = '{}'".format(self.current_user)).fetchone()[0]
		...
		ctrl(self.close_ecr_dialog, 'choice:stage').AppendItems(['New Request, needs reviewing', 'Reviewed, engineering in process', 'Change Complete, pending approval', 'Approved', 'Prototype Stage'])
		
		
		#add document options to choice box
		cursor.execute("SELECT document FROM ecr_document_choices")
		ctrl(self.close_ecr_dialog, 'choice:ecr_document').AppendItems(zip(*cursor.fetchall())[0])

		##self.ecr_type = 'Mechanical'

		self.close_ecr_dialog.Bind(wx.EVT_RADIOBUTTON, partial(Ecrs.radio_button_selected, type='Mechanical'), id=xrc.XRCID('radio:mechanical'))
		self.close_ecr_dialog.Bind(wx.EVT_RADIOBUTTON, partial(Ecrs.radio_button_selected, type='Electrical'), id=xrc.XRCID('radio:electrical'))
		self.close_ecr_dialog.Bind(wx.EVT_RADIOBUTTON, partial(Ecrs.radio_button_selected, type='Structural'), id=xrc.XRCID('radio:structural'))
		self.close_ecr_dialog.Bind(wx.EVT_RADIOBUTTON, partial(Ecrs.radio_button_selected, type='Other'), id=xrc.XRCID('radio:other'))
		...
		self.close_ecr_dialog.Bind(wx.EVT_CHOICE, Ecrs.on_choice_set_severity_default, id=xrc.XRCID('choice:ecr_document'))
		self.close_ecr_dialog.Bind(wx.EVT_CHOICE, Ecrs.on_choice_set_severity_default, id=xrc.XRCID('choice:ecr_component'))
		self.close_ecr_dialog.Bind(wx.EVT_CHOICE, Ecrs.on_choice_set_severity_default, id=xrc.XRCID('choice:ecr_sub_system'))

		#fill in the fields of the ecr we are modifing
		ecr = cursor.execute("SELECT reference_number, document, reason, type, request, resolution, when_needed, who_errored, priority, who_approved_first, who_approved_second, approval_stage, item, component, sub_system, severity FROM ecrs WHERE id = \'{}\'".format(close_ecr_id)).fetchone()
		...
		if ctrl(self.close_ecr_dialog, 'text:resolution').GetValue() == '':
			default_resolution = ''
			revisions = cursor.execute("SELECT document, description FROM revisions WHERE related_ecr='{}'".format(close_ecr_id)).fetchall()
		...
		ctrl(self.close_ecr_dialog, 'calendar:ecr_need_by').SetDate(wx.DateTimeFromDMY(need_by_date.tm_mday, need_by_date.tm_mon-1, need_by_date.tm_year))
		
		engineers = list(zip(*cursor.execute("SELECT name FROM employees WHERE department LIKE '%Engineering%' ORDER BY name ASC").fetchall())[0])
		engineers.insert(0, '')
	...
	def init_administration_tab(self):
		...
		cursor = Database.connection.cursor()
		cursor.execute('SELECT name FROM employees WHERE activated = 1 AND department = \'Design Engineering\' ORDER BY name ASC')
		ctrl(self.main_frame, 'choice:admin').AppendItems(zip(*cursor.fetchall())[0])
		
		#set current admin as current selection
		ctrl(self.main_frame, 'choice:admin').SetStringSelection(cursor.execute("SELECT ecr_admin_name FROM administration").fetchone()[0])

		self.main_frame.Bind(wx.EVT_CHOICE, Administration.on_select_admin, id=xrc.XRCID('choice:admin'))
		self.main_frame.Bind(wx.EVT_BUTTON, Administration.on_click_backup_database, id=xrc.XRCID('button:backup_database'))
	...
	def init_ecrs_tab(self):
		...
		self.main_frame.Bind(wx.EVT_BUTTON, Ecrs.on_click_add_revisions_with_ecr, id=xrc.XRCID('button:add_revisions_with_ecr'))
		...
		ctrl(self.main_frame, 'text:ecr_panel_description').SetBackgroundColour(ctrl(self.main_frame, 'notebook:main').GetBackgroundColour())
		ctrl(self.main_frame, 'text:ecr_panel_resolution').SetBackgroundColour(ctrl(self.main_frame, 'notebook:main').GetBackgroundColour())
		...
		self.main_frame.Bind(wx.EVT_BUTTON, Ecrs.refresh_my_assigned_ecrs_list, id=xrc.XRCID('button:my_assigned_ecrs_refresh'))
		...
		self.main_frame.Bind(wx.EVT_BUTTON, Ecrs.on_click_approve_selected_ers, id=xrc.XRCID('button:approve_selected_ecrs'))
		self.main_frame.Bind(wx.EVT_BUTTON, Ecrs.refresh_committee_ecrs_list, id=xrc.XRCID('button:committee_ecrs_refresh'))
	...
	def init_email_ecr_dialog(self):
		...
		order = cursor.execute("SELECT TOP 1 * FROM orders WHERE item = \'{}\'".format(ecr[3])).fetchone()
		ecr_details = ''
		if order != None:
		...
		self.email_ecr_dialog.Bind(wx.EVT_BUTTON, partial(destroy_dialog, dialog=self.email_ecr_dialog), id=xrc.XRCID('button:cancel_ecr_email'))
	...
	def init_new_ecr_dialog(self, duplicate_ecr_id = None):
		...
		try:
			##1/0.
			self.dbworks_connection = pyodbc.connect('DSN=DBWorks;UID=rmiller;APP=ECRev;WSID=SEN30;DATABASE=DBWorks_Kysor;Trusted_Connection=Yes')
		...
		self.new_ecr_dialog.Bind(wx.EVT_RADIOBUTTON, partial(Ecrs.radio_button_selected, type='Mechanical'), id=xrc.XRCID('radio:mechanical'))
		self.new_ecr_dialog.Bind(wx.EVT_RADIOBUTTON, partial(Ecrs.radio_button_selected, type='Electrical'), id=xrc.XRCID('radio:electrical'))
		self.new_ecr_dialog.Bind(wx.EVT_RADIOBUTTON, partial(Ecrs.radio_button_selected, type='Structural'), id=xrc.XRCID('radio:structural'))
		self.new_ecr_dialog.Bind(wx.EVT_RADIOBUTTON, partial(Ecrs.radio_button_selected, type='Other'), id=xrc.XRCID('radio:other'))
		...
		if duplicate_ecr_id != None:
			ecr = cursor.execute("SELECT reference_number, type, reason, document, request, resolution FROM ecrs WHERE id = \'{}\'".format(duplicate_ecr_id)).fetchone()
			...
			ctrl(self.new_ecr_dialog, 'calendar:ecr_need_by').SetDate(wx.DateTimeFromDMY(lead_time_date.day, lead_time_date.month-1, lead_time_date.year))


		ctrl(self.new_ecr_dialog, 'text:mentioned_parts').SetBackgroundColour(ctrl(self.main_frame, 'notebook:main').GetBackgroundColour())
	...
	def init_modify_ecr_dialog(self, modify_ecr_id):
		...
		ctrl(self.modify_ecr_dialog, 'choice:stage').AppendItems(['New Request, needs reviewing', 'Reviewed, engineering in process', 'Change Complete, pending approval', 'Approved', 'Prototype Stage'])
		...
		self.modify_ecr_dialog.Bind(wx.EVT_RADIOBUTTON, partial(Ecrs.radio_button_selected, type='Mechanical'), id=xrc.XRCID('radio:mechanical'))
		self.modify_ecr_dialog.Bind(wx.EVT_RADIOBUTTON, partial(Ecrs.radio_button_selected, type='Electrical'), id=xrc.XRCID('radio:electrical'))
		self.modify_ecr_dialog.Bind(wx.EVT_RADIOBUTTON, partial(Ecrs.radio_button_selected, type='Structural'), id=xrc.XRCID('radio:structural'))
		self.modify_ecr_dialog.Bind(wx.EVT_RADIOBUTTON, partial(Ecrs.radio_button_selected, type='Other'), id=xrc.XRCID('radio:other'))
		...
		self.modify_ecr_dialog.Bind(wx.EVT_CHOICE, Ecrs.on_choice_set_severity_default, id=xrc.XRCID('choice:ecr_document'))
		self.modify_ecr_dialog.Bind(wx.EVT_CHOICE, Ecrs.on_choice_set_severity_default, id=xrc.XRCID('choice:ecr_component'))
		self.modify_ecr_dialog.Bind(wx.EVT_CHOICE, Ecrs.on_choice_set_severity_default, id=xrc.XRCID('choice:ecr_sub_system'))
		
		#fill in the fields of the ecr we are modifing
		ecr = cursor.execute("SELECT reference_number, document, reason, type, request, resolution, when_needed, who_errored, priority, who_approved_first, who_approved_second, approval_stage, item, component, sub_system, severity FROM ecrs WHERE id = \'{}\'".format(modify_ecr_id)).fetchone()
		reference_number, document, reason, type, request, resolution, when_needed, who_errored, priority, who_approved_first, who_approved_second, approval_stage, item, component, sub_system, severity = ecr
		...
		need_by_date = time.strptime(str(ecr[6]), "%Y-%m-%d %H:%M:%S") #to python time object
		ctrl(self.modify_ecr_dialog, 'calendar:ecr_need_by').SetDate(wx.DateTimeFromDMY(need_by_date.tm_mday, need_by_date.tm_mon-1, need_by_date.tm_year))
	...
	def init_revisions_tab(self):
		...
		self.main_frame.Bind(wx.EVT_BUTTON, Revisions.on_click_add_new_revision, id=xrc.XRCID('button:add_new_revision'))
	...
	def init_search_tab(self):
		...
		self.joining_tables = []
		self.joining_tables.append(('ecrs', 'orders', 'item', 'item'))
		self.joining_tables.append(('ecrs', 'revisions', 'id', 'related_ecr'))
		self.joining_tables.append(('revisions', 'orders', 'item', 'item'))
		self.joining_tables.append(('revisions', 'ecrs', 'related_ecr', 'id'))
		self.joining_tables.append(('orders', 'revisions', 'item', 'item'))
		self.joining_tables.append(('orders', 'ecrs', 'item', 'item'))
		#self.joining_tables.append(('time_logs', 'orders', 'item', 'item'))
		...
		table_white_list = ['ecrs', 'orders', 'revisions']
		...
		self.main_frame.Bind(wx.EVT_BUTTON, ctrl(self.main_frame, 'list:search_results').export_list, id=xrc.XRCID('button:export_search_results'))
		self.main_frame.Bind(wx.EVT_BUTTON, ctrl(self.main_frame, 'list:search_results').print_list, id=xrc.XRCID('button:print_search_results'))		
	...
	def init_search_ecrs_dialog(self):
		if self.search_ecrs_dialog == None:
			...
			#populate some search field choices
			ctrl(self.search_ecrs_dialog, 'combo:search_value3').AppendItems(zip(*cursor.execute("SELECT document FROM ecr_document_choices").fetchall())[0])
			ctrl(self.search_ecrs_dialog, 'combo:search_value4').AppendItems(zip(*cursor.execute("SELECT reason FROM ecr_reason_choices").fetchall())[0])
			ctrl(self.search_ecrs_dialog, 'combo:search_value5').AppendItems(zip(*cursor.execute("SELECT department FROM departments").fetchall())[0])
			ctrl(self.search_ecrs_dialog, 'combo:search_value6').AppendItems(zip(*cursor.execute("SELECT name FROM employees ORDER BY name ASC").fetchall())[0])
			ctrl(self.search_ecrs_dialog, 'combo:search_value7').AppendItems(zip(*cursor.execute("SELECT focus FROM engineering_focuses").fetchall())[0])
			ctrl(self.search_ecrs_dialog, 'combo:search_value10').AppendItems(zip(*cursor.execute("SELECT name FROM employees ORDER BY name ASC").fetchall())[0])
			ctrl(self.search_ecrs_dialog, 'combo:search_value11').AppendItems(zip(*cursor.execute("SELECT name FROM employees ORDER BY name ASC").fetchall())[0])
			ctrl(self.search_ecrs_dialog, 'combo:search_value12').AppendItems(zip(*cursor.execute("SELECT name FROM employees ORDER BY name ASC").fetchall())[0])

			ctrl(self.search_ecrs_dialog, 'choice:sort_by').AppendItems(Database.get_table_column_names('ecrs', presentable=True))
			ctrl(self.search_ecrs_dialog, 'choice:sort_by').SetStringSelection('When requested')

			self.search_ecrs_dialog.Bind(wx.EVT_LIST_ITEM_SELECTED, Ecrs.on_select_ecr_item, id=xrc.XRCID('list:results'))
			...
			for i in range(11):
				self.search_ecrs_dialog.Bind(wx.EVT_CHOICE, partial(Ecrs.search_condition_selected, index=i), id=xrc.XRCID('choice:search_condition'+str(i)))
				self.search_ecrs_dialog.Bind(wx.EVT_TEXT, partial(Ecrs.search_value_entered, index=i), id=xrc.XRCID('combo:search_value'+str(i)))
	...
	def init_submit_revision_dialog(self, item_entries):
		...
		self.new_revision_dialog.Bind(wx.EVT_BUTTON, Revisions.on_click_submit_revision, id=xrc.XRCID('button:submit_revision'))
		self.new_revision_dialog.Bind(wx.EVT_CHOICE, Revisions.on_select_revision_reason, id=xrc.XRCID('choice:revision_reasons'))
		...
		if ecr_reason != 'BOM Reconciliation':
			try: reasons.remove('BOM Reconciliation')
