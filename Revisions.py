...
def on_click_add_new_revision(event):
	...
	if related_ecr != '':
		...
		if not cursor.execute('SELECT TOP 1 id FROM ecrs WHERE id = \'{}\''.format(related_ecr)).fetchone():
			wx.MessageBox('The related ECR ID: {} was not found in the database.\n\nCorrect the ECR ID or clear that field to revise\nwithout relating the revision to an ECR.'.format(related_ecr), 'Error', wx.OK | wx.ICON_WARNING)
			return
...
def on_click_submit_revision(event):
	...
	for row in range(table.GetNumberRows()):
		...
		if document != '' and description == '':
			wx.MessageBox('You must enter a change description for document: {}'.format(document), 'Hint', wx.OK | wx.ICON_WARNING)
			return
	...
	for new_revision_record in new_revision_records:
		document_reminder = cursor.execute('SELECT TOP 1 submission_reminder FROM revision_document_choices WHERE document = \'{}\''.format(new_revision_record[0])).fetchone()
	...
	for item_entry in item_entries:
		for new_revision_record in new_revision_records:
			level = cursor.execute('SELECT TOP 1 level FROM revisions WHERE item = \'{}\' AND document = \'{}\' ORDER BY when_revised DESC'.format(item_entry, new_revision_record[0])).fetchone()
			if level == None:
				level = 1
			else:
				level = level[0] + 1
			...
			sql += '\'{}\', '.format(ctrl(General.app.new_revision_dialog, 'choice:revision_reasons').GetStringSelection())
			...
			cursor.execute('UPDATE revisions SET dollars_reconciled = NULL WHERE id = \'{}\' AND dollars_reconciled = 0'.format(latest_id))
			cursor.execute('UPDATE revisions SET related_ecr = NULL WHERE id = \'{}\' AND related_ecr = 0'.format(latest_id))
			
			
			revision = cursor.execute('SELECT TOP 1 * FROM revisions WHERE id = \'{}\''.format(latest_id)).fetchone()
			order = cursor.execute('SELECT TOP 1 * FROM {} WHERE item = \'{}\''.format(Ecrs.table_used, revision[1])).fetchone()
	...
	if event.GetCol() == 0:
		...
		for row in range(table.GetNumberRows()):
			if document == table.GetCellValue(row, 0).strip():
				wx.MessageBox('You already entered a revision for document: {}'.format(document), 'Hint', wx.OK | wx.ICON_WARNING)
				return
...
def on_entry_revision_items(event):
	...
	for entry in entries:
		...
		if entry != '':
			...
			order = cursor.execute("SELECT TOP 1 sales_order FROM {} WHERE item LIKE '%{}%'".format(Ecrs.table_used, entry)).fetchone()
			if order != None:
				if len(entry) == 9 or \
					(len(entry) == 7 and entry[:2] == '01') or \
					(len(entry) == 8 and entry[:2] != 'KW') or \
					len(entry) == 18:
					entry = cursor.execute("SELECT TOP 1 item FROM {} WHERE item LIKE '%{}%'".format(Ecrs.table_used, entry)).fetchone()[0]
	...
	for page_label in pages_to_add:
		...
		list_control = wx.ListCtrl(panel, -1, style=wx.LC_REPORT|wx.BORDER_NONE|wx.LC_VRULES|wx.LC_HRULES|wx.LC_SINGLE_SEL)
		...
		revisions = cursor.execute("SELECT * FROM revisions WHERE item LIKE '%{}%' ORDER BY when_revised DESC".format(page_label.split(' ')[0])).fetchall()
...
def onCharEvent(event):	
	...
	if keycode == 5:
		...
		wx.MessageBox("A copy of this revision notice has been sent to {}".format(users_email), 'Info', wx.OK | wx.ICON_INFORMATION)
...
def send_revision_email(revision, order, email_list, sender):
	...
	body_html = '''<style type=\"text/css\">td{{font-family:Arial; color:black; font-size:12pt;}}</style>
		...
		'''.format(shortcuts, item_number, sales_order, customer, location, model, target_date, produced_date, revision_id, related_ecr, reason, document, level, description)
