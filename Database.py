...
def connect_to_database():
	...
	if config.read('ECRev.cfg'):
		...
		wx.MessageBox('Could not locate \'ECRev.cfg\' in the ECRev folder.\nUnable to read database connection string.', 'ERROR', wx.OK | wx.ICON_ERROR)
		return False
...
def get_order_data_from_ref(reference_number):
	...
	try:
		...
		if (len(reference_number) == 7) and (reference_number[0] == '0'):
			...
			return cursor.execute("SELECT * FROM orders WHERE item = \'{}\' LAMIT 1".format(reference_number)).fetchone()
			...
			return cursor.execute("SELECT * FROM orders WHERE serial = \'{}\' LAMIT 1".format(reference_number)).fetchone()

		elif len(reference_number) == 6:
			#it might be a sales order, search for it in the DB
			return cursor.execute("SELECT * FROM orders WHERE sales_order = \'{}\' OR quote = \'{}\' LAMIT 1".format(reference_number, reference_number)).fetchone()
			...
			return cursor.execute("SELECT * FROM orders WHERE sales_order = \'{}\' AND line_up = \'{}\' LAMIT 1".format(sales_order, line_up)).fetchone()
...
def get_item_from_ref(reference_number):
	...
	try:
		...
		if (len(reference_number) == 7) and (reference_number[0] == '0'):
			#it might be an item number, search for it in the DB
			item_number = cursor.execute("SELECT TOP 1 * FROM orders.root LIKE '%{}%'".format(reference_number)).fetchone()
			...
			#it might be a serial number, search for it in the DB
			item_number = cursor.execute("SELECT TOP 1 * FROM orders.root WHERE serial = \'{}\'".format(reference_number)).fetchone()

		elif len(reference_number) == 6:
			#it might be a sales order, search for it in the DB
			item_number = cursor.execute("SELECT TOP 1 * FROM orders.root WHERE sales_order LIKE '%{}%' OR quote = \'{}\'".format(reference_number, reference_number)).fetchone()
			...
			if reference_number.split('-')[1] != '':
				line_up = int(float(reference_number.split('-')[1]))
			item_number = cursor.execute("SELECT TOP 1 * FROM orders.root WHERE sales_order LIKE '%{}%' AND line_up = \'{}\'".format(sales_order, line_up)).fetchone()
