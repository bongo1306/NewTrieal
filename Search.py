...
def export_search_results(event):
	...
	save_dialog = wx.FileDialog(General.app.main_frame, message="Export file as ...", 
							...
							defaultFile="search_results", wildcard="Excel Spreadsheet (*.xls)|*.xls", style=wx.SAVE|wx.OVERWRITE_PROMPT)
...
def on_click_begin_search(event):
	...
	for dangerous_sql_command in dangerous_sql_commands:
		if dangerous_sql_command in sql.upper():
			wx.MessageBox('{} is not permitted in the search query.'.format(dangerous_sql_command), 'Warning', wx.OK | wx.ICON_WARNING)
			return
...
def on_click_open_how_to_search(event):
	...
	how_to_search_frame.Bind(wx.EVT_BUTTON, partial(destroy_dialog, dialog=how_to_search_frame), id=xrc.XRCID('button:close'))
		...
		if split_by_token.upper().strip() in tokens:
				...
				for operator in operators:
					if seperated_criteria == operator:
						if seperated_criteria_list[seperated_index+1] not in operators:
							seperated_criteria_list[seperated_index+1] = seperated_criteria_list[seperated_index+1].strip()
			...
			for seperated_index, seperated_criteria in enumerate(reversed(seperated_criteria_list)):
				for operator in operators:
					if seperated_criteria == operator:
						if seperated_criteria_list[seperated_index-1] not in operators:
							seperated_criteria_list[seperated_index-2] = ' {} '.format(seperated_criteria_list[seperated_index-2])
